...
export default async function handler(req, res) {
  ...
  try {
    const { nombre, email, mensaje, telefono, codigoPais, tieneWhatsApp } = req.body;
    ...
    const phoneInfo = telefono 
      ? `${codigoPais} ${telefono}${tieneWhatsApp ? ' (WhatsApp disponible)' : ''}`
      : '';

    const emailContent = createTrixEmailTemplate({
      nombre,
      mensaje,
      telefono: phoneInfo
    });
...
const createTrixEmailTemplate = ({ nombre, mensaje, telefono }) => {
  return `
<!DOCTYPE html>
<html lang="es">
<head>
